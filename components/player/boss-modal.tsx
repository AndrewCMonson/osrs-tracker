...
import { X, Trophy, Check, Circle } from 'lucide-react';
import { BossName, BOSS_DISPLAY_NAMES, BossData, KC_THRESHOLDS, formatKc } from '@/types/bosses';
      ...
      >
        ...
        <div className="flex items-center gap-4 p-5 border-b border-stone-800">
          ...
          <div className="flex-1 min-w-0">
            <h2 id="boss-modal-title" className="text-lg font-bold text-stone-100 truncate">
              {displayName}
            </h2>
        ...
        <div className="p-5">
          ...
          <div className="grid grid-cols-3 sm:grid-cols-4 gap-2">
            {KC_THRESHOLDS.map((threshold) => {
              ...
              const isNext = !isAchieved && KC_THRESHOLDS.filter(t => data.killCount >= t).length === KC_THRESHOLDS.indexOf(threshold);
                  ...
                  <div className="absolute -top-1.5 -right-1.5">
                    {isAchieved ? (
                      ...
                      <div className="w-5 h-5 rounded-full bg-stone-700 flex items-center justify-center">
                        <Circle className="h-3 w-3 text-stone-500" strokeWidth={2} />
                      </div>
                    )}
                  </div>
                  <span className={cn(
                    'text-sm font-bold',
                    isAchieved ? 'text-amber-500' : isNext ? 'text-stone-200' : 'text-stone-500'
                  )}>
          ...
          {data.killCount < KC_THRESHOLDS[KC_THRESHOLDS.length - 1] && (
            <div className="mt-5 pt-4 border-t border-stone-800">
              {(() => {
                const nextMilestone = KC_THRESHOLDS.find(t => data.killCount < t);
                if (!nextMilestone) return null;
                const prevMilestone = KC_THRESHOLDS[KC_THRESHOLDS.indexOf(nextMilestone) - 1] || 0;
                const progress = ((data.killCount - prevMilestone) / (nextMilestone - prevMilestone)) * 100;
                const remaining = nextMilestone - data.killCount;

                return (
                  <>
                    <div className="flex justify-between text-xs text-stone-400 mb-2">
                      <span>Progress to {nextMilestone.toLocaleString()} KC</span>
